...
unsigned long long portable_network_graphics_decode(unsigned char* source_address, unsigned char* destination_address)
{
  ...
  while (1)
  {
    ...
    {
      portable_network_graphics_copy(source_address, chunk_length, decode_address);
  ...
  decode_address -= decode_size;

  // Translate contiguous IDAT according to IHDR.
